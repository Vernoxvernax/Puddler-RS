use std::{thread::{self}, time::{Duration, SystemTime}};
...
use crate::{discord::DiscordClient, input::clear_stdin, media_center::{Item, MediaCenter, PlaybackInfo}, media_config::MediaCenterType, printing::{print_message, PrintMessageType}, puddler_settings::PuddlerSettings, APPNAME};
...
impl Player {
  ...
  pub fn set_plex_video(&mut self, item: PlexItem, server_address: String, auth: String, transcoding_settings: &mut Option<(bool, Option<u32>, Option<u32>, String)>) {
    ...
    let stream_url = if handle.config.transcoding {
      ...
      format!(
        "{}{}?{}&X-Plex-Platform={}",
        server_address, item.Media.clone().unwrap()[0].Part[0].key.trim_start_matches('/'), auth, urlencoding::encode("Plex Home Theater")
      )
    };

    let mut commands: Vec<[String; 4]> = vec![];
    if !handle.config.transcoding {
      for stream in item.Media.clone().unwrap()[0].Part[0].Stream.clone().unwrap() {
        if let Some(key) = &stream.key {
          let media_url = format!("{}{}?{}",
            server_address, key.trim_start_matches('/'), auth
          );
  ...
  pub fn set_jellyfin_video(&mut self, item: Item, playback_info: PlaybackInfo, server_address: String, auth_token: String, transcoding_settings: &mut Option<(bool, Option<u32>, Option<u32>, String)>) {
    ...
    let stream_url = if let Some(transcoding_url) = &media_source.TranscodingUrl {
      format!("{}{}", server_address, transcoding_url.trim_start_matches('/'))
    } else {
      format!("{}Videos/{}/stream?Container=mkv&Static=true&api_key={}", server_address, item.Id, auth_token)
    };
  ...
  #[tokio::main]
  pub async fn play(&mut self) -> Video { // any time vars in here are in seconds
    let mut video: Video;
    ...
    if handle.config.media_center_type != MediaCenterType::Plex {
      ...
      let address: String = http_address.trim_start_matches("https").trim_start_matches("http").trim_end_matches('/').to_string();
      ...
      let url = format!("{}{}/socket?api_key={}&deviceId={}", protocol, address, token, handle.get_device_id());
    ...
    let media_title = format!("{} | {}", video.title[0], config.media_center_type.to_string());
    let mpv_title = format!("{} - Streaming: {} ({})", APPNAME, video.title[0], config.media_center_type.to_string());
    ...
    mpv.set_property("force-media-title", media_title).expect("Failed to configure force-media-title.");
    mpv.set_property("title", mpv_title).expect("Failed to configure title.");

    let mut ctx = mpv.create_event_context();
    ctx.disable_deprecated_events().expect("Failed to disable deprecated events.");

    mpv.command("loadfile", &[&video.stream_url]).expect("Failed to load file.");

    media_center.start_playback(video.clone().id, video.playback_position);

    for shader in &self.settings.glsl_shaders {
      mpv.command("change-list", &["glsl-shaders", "append", shader.as_str()]).expect("Failed to add glsl-shader file");
    }
        ...
        match event {
          Event::FileLoaded => {
            if resume_progress != 0 && !config.transcoding {
              mpv.command("seek", &[&resume_progress.to_string()]).expect("Failed to seek");
            }
        ...
        if current_time > old_pos + 15.0 {
          ...
          if self.settings.discord_presence {
            if video.video_type == VideoType::Movie {
              discord.update_presence(config.media_center_type,
                "".to_string(),
                video.title[0].clone(),
          ...
          if self.settings.discord_presence {
            if video.video_type == VideoType::Movie {
              discord.pause(config.media_center_type,
                "".to_string(),
                video.title[0].clone()
              );
            } else {
...
fn load_external_subtitles(video: Video, mpv: &Mpv) {
  if let Some(commands) = video.external_media {
    for command in commands {
      ...
      mpv.command("sub-add", &[&media_url, &auto, &formatted_title, &language]).unwrap();
    }
  }
}
...
async fn websocket_read(
  ...
  if let Some(mut socket) = faucet { // can't get any more creative than that
  ...
  if let Some(mut socket) = sink {
    while let Some(msg) = web_output.recv().await {
      ...
      if let Err(err) = socket.send(Message::Text(msg)).await {
        print_message(PrintMessageType::Error, format!("Failed to send message through websocket: {}", err).as_str())
      };
    }
  }
}
