...
use crate::{input::{interactive_menuoption, interactive_select, InteractiveOption, InteractiveOptionType}, media_center::{set_config, MediaCenter}, media_config::{Config, MediaCenterType, Objective}, printing::{print_message, PrintMessageType}};
    ...
    match interactive_menuoption(options.clone()) {
      MenuOptions::Default => {
        ...
        if let Err(e) = handle.read() {
          print_message(PrintMessageType::Error, format!("Failed reading config file: {:?}", e).as_str());
          continue;
        }
      },
      MenuOptions::Choose => {
        if let Ok(configs) = handle.new() {
          ...
          for config in configs.clone() {
            configs_str.append(&mut vec![InteractiveOption {
              text: format!("{} - {}:Modify", config.config.server_name, config.config.media_center_type.to_string()),
